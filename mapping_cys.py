...
"""
This script generates the cys residues IDs (chainID_resi_resn) within 4A of ligand, exports as a csv in the current directory.
The csv contains in each row:
pdb_id: [*residue_id]
"""
...
for filename in os.listdir():
    if filename.endswith(".pdb"):
        ...
        res_list = sorted(list(cys_global_set))
        chains_string = "_".join(sorted(list(set(res.split("_")[0] for res in res_list))))

        mapping_dict[filename[0:-4]] = [chains_string] + res_list
        cmd.delete(filename[0:-4])

max_length = max([len(v) for v in mapping_dict.values()])
for pdb_id, res_id in mapping_dict.items():
    mapping_dict[pdb_id] = res_id + [None] * (max_length - len(res_id))
mapping_df = pd.DataFrame(mapping_dict).transpose()
mapping_df.to_csv("pdb_cys.csv", sep=",")
